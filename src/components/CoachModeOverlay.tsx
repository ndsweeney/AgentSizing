...
export function CoachModeOverlay({ view }: CoachModeOverlayProps) {
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 bg-indigo-600 text-white p-3 rounded-full shadow-lg hover:bg-indigo-700 transition-all animate-in fade-in slide-in-from-bottom-4"
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-96 bg-white rounded-xl shadow-2xl border-2 border-indigo-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4 flex flex-col">
      {/* Header */}
      <div className="bg-indigo-600 p-4 flex items-center justify-between text-white">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs bg-indigo-500 px-2 py-0.5 rounded text-indigo-100">
      ...
      {/* Content */}
      <div className="p-6 bg-gradient-to-b from-indigo-50/50 to-white min-h-[200px]">
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            {currentTip.type === 'question' && <HelpCircle className="w-4 h-4 text-blue-600" />}
            {currentTip.type === 'steering' && <MessageCircle className="w-4 h-4 text-green-600" />}
            {currentTip.type === 'pitfall' && <AlertTriangle className="w-4 h-4 text-red-600" />}
            {currentTip.type === 'tip' && <Lightbulb className="w-4 h-4 text-yellow-600" />}
            
            <span className={cn(
              "text-xs font-bold uppercase tracking-wider",
              currentTip.type === 'question' && "text-blue-600",
              currentTip.type === 'steering' && "text-green-600",
              currentTip.type === 'pitfall' && "text-red-600",
              currentTip.type === 'tip' && "text-yellow-600",
            )}>
              {currentTip.type}
            </span>
          </div>
          <h4 className="font-bold text-gray-900 text-lg mb-2">{currentTip.title}</h4>
          ...
          {relevantContent.map((_, idx) => (
            <div 
              ...
              className={cn(
                ...
                idx === activeTipIndex ? "bg-indigo-600" : "bg-gray-300"
              )}
            />
          ))}
        </div>
