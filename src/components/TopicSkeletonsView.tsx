    ...
    <div className="flex h-[600px] border border-gray-200 rounded-xl overflow-hidden bg-white">
          ...
          {skeletons.map((sk, idx) => (
            ...
            >
              ...
              <div className="overflow-hidden">
                ...
                <div className="text-xs text-gray-400 truncate">{sk.agentType}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-white">
          <div>
            <h2 className="font-semibold text-gray-900">{activeSkeleton.name}</h2>
            <p className="text-xs text-gray-500">{activeSkeleton.agentType}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              {copiedMd ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"
            >
              {copiedJson ? <Check className="w-3.5 h-3.5 text-green-600" /> : <FileJson className="w-3.5 h-3.5" />}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          <div className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-sm border border-gray-200">
            <div className="prose prose-sm max-w-none">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Trigger Phrases</h3>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {activeSkeleton.variables.map((v, i) => (
                  <span key={i} className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-mono border border-blue-100">
                    {v}
                  </span>
                ))}
              </div>
              ...
              <ol className="list-decimal pl-5 mb-6 text-gray-600 space-y-1">
                {activeSkeleton.steps.map((step, i) => (
                  <li key={i} className="pl-1">{step.replace(/^\d+\.\s*/, '')}</li>
                ))}
              </ol>

              <h3 className="text-lg font-semibold text-gray-900 mb-4">Actions & Skills</h3>
              <div className="flex flex-wrap gap-2 mb-6">
                {activeSkeleton.actions.map((action, i) => (
                  <span key={i} className="px-2 py-1 bg-purple-50 text-purple-700 rounded text-xs font-mono border border-purple-100">
                    {action}
                  </span>
                ))}
              </div>
              ...
              <div className="space-y-2 mb-6">
                {activeSkeleton.responseTemplates.map((tmpl, i) => (
                  <div key={i} className="p-3 bg-gray-50 rounded border border-gray-100 text-gray-600 italic text-sm">
                    "{tmpl}"
                  </div>
                ))}
              </div>
              ...
              {activeSkeleton.guardrailNotes && activeSkeleton.guardrailNotes.length > 0 && (
                <>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Guardrails & Notes</h3>
