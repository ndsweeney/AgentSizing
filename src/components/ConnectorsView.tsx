  ...
  if (connectors.length === 0) {
    return (
      <div className="p-8 text-center bg-gray-50 rounded-xl border border-gray-200">
        ...
        <h3 className="text-lg font-medium text-gray-900">No Specific Connectors Identified</h3>
...
function ConnectorCard({ connector }: { connector: ConnectorDefinition }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="p-6">
        <p className="text-sm text-gray-600 mb-6">{connector.description}</p>

        <div className="grid lg:grid-cols-3 gap-6">
          {/* Schema List */}
          <div className="lg:col-span-1 space-y-2">
            <h5 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Available Actions</h5>
            {connector.schemas.map(schema => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className={cn("font-medium", expandedSchema === schema.name ? "text-blue-900" : "text-gray-900")}>
                    {schema.name}
                  </span>
                  <span className={cn(
                    "text-[10px] font-mono px-1.5 py-0.5 rounded border",
                    schema.method === 'GET' ? "bg-green-50 text-green-700 border-green-200" :
                    schema.method === 'POST' ? "bg-blue-50 text-blue-700 border-blue-200" :
                    "bg-gray-50 text-gray-600 border-gray-200"
                  )}>
                    {schema.method}
                  </span>
                </div>
                <p className="text-xs text-gray-500 truncate">{schema.description}</p>
              </button>
            ))}
          </div>

          {/* Schema Details */}
          <div className="lg:col-span-2 bg-gray-900 rounded-xl overflow-hidden flex flex-col">
            {connector.schemas.map(schema => {
                ...
                <div key={schema.name} className="flex flex-col h-full">
                  <div className="px-4 py-3 bg-gray-800 border-b border-gray-700 flex items-center justify-between">
                    <div className="flex items-center gap-2 overflow-hidden">
                      ...
                      <code className="text-xs text-gray-300 font-mono truncate" title={schema.endpoint}>
                        {schema.endpoint}
                      </code>
                    </div>
                  ...
                  <div className="flex-1 overflow-auto p-4 custom-scrollbar">
                    <div className="space-y-4">
                      {schema.requestSchema && (
                        <div>
                          <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Request Body Schema</div>
                          <pre className="text-xs font-mono text-blue-300 bg-gray-950/50 p-3 rounded-lg border border-gray-800 overflow-x-auto">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Response Example</span>
                          <span className="text-[10px] text-gray-600">JSON</span>
                        </div>
                        <pre className="text-xs font-mono text-green-300 bg-gray-950/50 p-3 rounded-lg border border-gray-800 overflow-x-auto">
